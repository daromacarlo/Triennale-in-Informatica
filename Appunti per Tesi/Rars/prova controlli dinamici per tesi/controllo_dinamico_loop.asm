.globl controllo_dinamico_loop
.globl REGISTRI_VIRTUALI        # Esporta il simbolo per gli altri file

.data
    .align 4
    programma: .ascii ""
    fineprogramma:
    testo_ok: .asciz "tutto a posto"
    testo_errore: .asciz "successo casotto"
    MEMORIA_VIRTUALE: .space 4096  # 4KB di memoria simulata possono bastare per provare progetti didattici.
    
    REGISTRI_VIRTUALI: .space 128  # 32 registri da 4 byte, inizializzati a 0
    CONTAPASSI:    .word 0
    MAXPASSI:      .word 10000000

.text
controllo_dinamico_loop:
    la a1, MEMORIA_VIRTUALE
    la a5, REGISTRI_VIRTUALI
    la s2, programma                 # s2 = puntatore istruzione corrente #s2 è il mio PC_SIMULATO
    la s3, fineprogramma 
    li s4, 0                         # s4 = contatore passi
    la s5, MAXPASSI
    lw s5, 0(s5)
ciclo_simulazione:
   while:
      addi s4, s4, 1
      bge s2, s3, ok			            	
      bgt s4, s5, errore_loop_infinito

      # Lettura e conversione della stringa HEX in t6 (logica originale mantenuta)
      lbu t1, (s2)
      addi t1, t1, -48
      ble t1, zero, continua1 # Usato zero invece di s1 se non inizializzato
      addi t1, t1, -39
    continua1:
      lbu t2, 1(s2)
      addi t2, t2, -48
      ble t2, zero, continua2
      addi t2, t2, -39
    continua2:
      lbu t3, 2(s2)
      addi t3, t3, -48
      ble t3, zero, continua3
      addi t3, t3, -39
    continua3:
      lbu t4, 3(s2)
      addi t4, t4, -48
      ble t4, zero, continua4
      addi t4, t4, -39
    continua4:
      lbu t5, 4(s2)
      addi t5, t5, -48
      ble t5, zero, continua5
      addi t5, t5, -39
    continua5:
      lbu s9, 5(s2)
      addi s9, s9, -48
      ble s9, zero, continua6
      addi s9, s9, -39
    continua6:
      lbu s10, 6(s2)
      addi s10, s10, -48
      ble s10, zero, continua7
      addi s10, s10, -39
    continua7:
      lbu s11, 7(s2)
      addi s11, s11, -48
      ble s11, zero, continua8
      addi s11, s11, -39
    continua8:

      # Ricostruzione della word in t6
      li t6, 0
      add t6, t6, s10
      slli t6, t6, 4
      add t6, t6, s11
      slli t6, t6, 4
      add t6, t6, t5
      slli t6, t6, 4
      add t6, t6, s9
      slli t6, t6, 4
      add t6, t6, t3
      slli t6, t6, 4
      add t6, t6, t4
      slli t6, t6, 4
      add t6, t6, t1
      slli t6, t6, 4
      add t6, t6, t2

      andi s10, t6, 127               # Estraggo Opcode
      
      li s11, 99                     
      beq s10, s11, simula_B
      li s11, 51                      
      beq s10, s11, simula_R
      
      # (Qui andrebbero gli altri opcode...)
      
      j while
     

errore_loop_infinito:
    la a0, testo_errore
    li a7, 4
    ecall
    li a7, 10
    ecall

ok:
    la a0, testo_ok
    li a7, 4
    ecall
    li a7, 10
    ecall
