...
void Customer::doTransition() {
    ...
    if (!s_candidates.empty()) {
        s = s_candidates[std::uniform_int_distribution<int>(0, s_candidates.size()-1)(RNG)];
    }
    ...
    if (!p_candidates.empty()) {
        i_prod = p_candidates[std::uniform_int_distribution<int>(0, p_candidates.size()-1)(RNG)];
    }
    ...
    if (!q_candidates.empty()) {
        q_req = q_candidates[std::uniform_int_distribution<int>(0, q_candidates.size()-1)(RNG)];
    }
