...
Server::Server(int id, DB* db_ref) : id(id), db(db_ref) {
    cache.resize(PARAMS.P + 1);
    sellBuy = 0.0;
    y = 0.0;
}

// Implementazione fedele della formula discreta ogni T secondi
void Server::updateCostPeriodic() {
    double current_cost = 0;
    for (int i = 1; i <= PARAMS.P; ++i) {
        current_cost += cache[i].q * cache[i].v; // alpha(s, i) = q * v
    }
    
    // y(s, t+T) = y(s, t) + T * 0.1 * sum(alpha)
    y += PARAMS.T * 0.1 * current_cost; 

    // Schedula il prossimo aggiornamento tra T secondi
...
void Server::triggerCacheUpdate() {
    ...
    int i_req = 1;
    bool req_even_i = dist01(RNG) < prob_i;
    ...
    int q_req = 1;
    bool req_even_q = dist01(RNG) < prob_q;
